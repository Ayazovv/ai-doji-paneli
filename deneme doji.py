    ...
    try:
        ...
        if market["category"] == "Emtia":
            try:
                ...
                # Önbelleğe (cache) takılmamak için veri_indir fonksiyonunu es geçip direkt yf.download kullanıyoruz!
                spot_df = yf.download(spot_sym, period="1d", interval="1m", progress=False) 
...
valid_signals = dict(
    sorted(
        ham_sinyaller.items(), 
        key=lambda x: (
            x[1]["result"].get("reboundPct", 0.0),  
            x[1]["result"].get("drawdownPct", 0.0)  
        ), 
        reverse=True
    )
)
    ...
    for sym, data in valid_signals.items():
        ...
        with st.container(border=True):
            ...
            if st.button(f"📊 Grafiği İncele", key=f"chart_btn_{m['symbol']}_{m['category']}"):
                ...
                st.rerun()        vix_df = yf.download("^VIX", period="5d", interval="1d", progress=False)
        if isinstance(vix_df.columns, pd.MultiIndex):
            vix_df.columns = vix_df.columns.get_level_values(0)
        son_vix = float(vix_df['Close'].iloc[-1])
        fng_score = 100 - ((son_vix - 10) / (40 - 10) * 100)
