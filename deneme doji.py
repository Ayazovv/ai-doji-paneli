...
def analiz_et_safe(market, min_hours, interval, doji_modu):
    try:
        ...
        # BUY hedefi: ileri kapanis %1 yukari cikti VE stop %0.99 altina dusmediyse 1
        buy_target = np.where(
            (ilerideki_kapanis > suanki_fiyat * 1.01) &
            (ilerideki_min >= suanki_fiyat * 0.99),
            1, 0
        )
        # SELL hedefi: ileri kapanis %1 asagi dustuyse 1
        sell_target = np.where(
            ilerideki_kapanis < suanki_fiyat * 0.99,
            1, 0
        )
        # Nihai hedef: BUY=1, SELL=0, ikisi de yoksa -1 (kararsiz mumlar eleniyor)
        ...
        # Doji tipi gölge uzunluklarına göre belirleniyor (RSI ile ilgisi yok)
        ...
        if upper_shadow_val > 0.6 and lower_shadow_val < 0.2:
            doji_type = "Gravestone"  # Uzun üst gölge, kısa alt gölge
        elif lower_shadow_val > 0.6 and upper_shadow_val < 0.2:
            doji_type = "Dragonfly"   # Uzun alt gölge, kısa üst gölge
        else:
            doji_type = "Standard"    # Dengeli gölgeler
        big_trend = buyuk_trend_kontrol(market["symbol"])
        
        # --- DÜZELTİLEN KISIM: Hesaplamayı sözlükten dışarı (yukarı) aldık ---
        # Degisim: 1h->gunluk(24 mum), 4h->haftalik(30 mum), 1d->haftalik(5 mum)
