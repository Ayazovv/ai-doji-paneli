# -*- coding: utf-8 -*-
"""
AI Doji Terminali - v6 (Pro XGBoost, EMA200, Özel Hedef & Tam Arayüz)
"""
...
def analiz_et_safe(market, min_hours, interval, doji_modu):
    try:
        ...
        # --- KULLANICININ ÖZEL HEDEF FİLTRESİ (KARARSIZ MUMLARI ELEME) ---
        ...
        feature_names_tr = {
            'RSI': 'RSI (Aşırılık)', 'Price_to_EMA20': 'Kısa Trend (20)', 'Price_to_EMA200': 'Ana Trend (200)',
        ...
        doji_tarihi = "Tarih Yok"
        if olgun_dojiler:
            gecen_mum = min(olgun_dojiler) 
            hedef_idx = len(df) - 1 - gecen_mum
            doji_tarihi = df.index[hedef_idx].strftime("%d-%m-%Y %H:%M")
        else:
            gecen_mum = 0 
            doji_tarihi = df.index[-1].strftime("%d-%m-%Y %H:%M")
            
        X = df[ozellikler].iloc[:-int(min_hours)]
        ...
        if len(X) > min_required_len:
            ...
            if not doji_test_df.empty:
                X_test_doji = doji_test_df[ozellikler]
            ...
            if hasattr(model, 'feature_importances_'):
                ...
                sirali = sorted(zip(ozellikler, importances), key=lambda x: x[1], reverse=True)
        ...
        # --- KULLANICININ ÖZEL DEĞİŞİM (CHANGE) MANTIĞI ---
...
<div style='text-align: center; padding: 10px; border-bottom: 1px solid #1E293B; margin-bottom: 20px;'>
    <h3 style='color: #FFF; margin: 0; font-size: 16px;'>🤖 AI TERMINAL v6 (Pro)</h3>
</div>
""", unsafe_allow_html=True)
...
if not valid_signals:
    ...
    for sym, data in valid_signals.items():
            ...
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div style="flex: 1;">
                    ...
                    <div style="color: #94A3B8; font-size: 13px; margin-top: 4px;">
                        ⏰ <b>Doji Yaşı: {h_ago} Mum</b> <span style="color: #CBD5E1;">(Tarih: <b>{d_date}</b> • {d_type} Doji)</span> • <span style="color: #CBD5E1;">Büyük Trend (4h): <b>{b_trend}</b></span>
                    </div>
        ...
        """.format(
            ...
            d_date=r['dojiDate'], d_type=r['dojiType'], b_trend=r['bigTrend'], b_bg=badge_bg, sig=r['signal'],
